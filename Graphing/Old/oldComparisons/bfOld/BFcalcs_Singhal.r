...
#writeCSV(toxSupport,"toxdGLSavg_support.csv")
...
#writeCSV(allBF.SC.loci,"Singhal.allBF.SC.loci.csv")
#writeCSV(allBF.AI.loci,"Singhal.allBF.AI.loci.csv")
#writeCSV(allBF.SA.loci,"Singhal.allBF.SA.loci.csv")
#writeCSV(allBF.SI.loci,"Singhal.allBF.SI.loci.csv")
...
#writeCSV(toxBF.AI.loci,"Singhal.toxBF.AI.loci.csv")
#writeCSV(toxBF.SA.loci,"Singhal.toxBF.SA.loci.csv")
#writeCSV(toxBF.SI.loci,"Singhal.toxBF.SI.loci.csv")
...
ggsave("Singhal_BF_Violin_all.jpg", plot=a, width = 40, height = 20, units = "cm")
...
ggsave("Singhal_BF_Violin_tox.jpg", plot=t, width = 40, height = 20, units = "cm")
...
all.trunc <- ggplot(allBF, aes(x=BF, fill=Hypothesis, color=Hypothesis)) + 
  ...
  xlim(-100,100) + scale_color_manual(values=myList)+scale_fill_manual(values=myList)

all.trunc 

a <- ggarrange(all,all.trunc, ncol=1, nrow=2, common.legend = TRUE, legend="right") 
ggsave("Singhal_BF_Histo_all.jpg", plot=a, width = 40, height = 20, units = "cm")
...
tox.trunc <- ggplot(toxBF, aes(x=BF, fill=Hypothesis, color=Hypothesis)) + 
  geom_histogram(binwidth = 5, alpha=0.5, position="dodge") +
  geom_vline(xintercept=c(0,10),color=c("grey55", "black"), linetype="dashed", size=0.5) + 
  xlim(-100,100) + scale_color_manual(values=txList)+scale_fill_manual(values=txList)

t <- ggarrange(tox,tox.trunc, ncol=1, nrow=2, common.legend = TRUE, legend="right") 
t
ggsave("Singhal_BF_Histo_tox.jpg", plot=a, width = 40, height = 20, units = "cm")
...
sc <- ggplot(ScleroBF, aes(fill=Hypothesis, x=BF, color=Hypothesis)) + 
  ...
  theme_bw() + theme(panel.border = element_blank()) + 
  coord_cartesian(xlim=c(-450,300)) + scale_x_continuous(breaks=seq(-450,300,100)) +
...
ggsave("Singhal_BF_Histo_ScleroPairwise.jpg", plot=a, width = 20, height = 20, units = "cm")
...
all
ggsave("Singhal_BF_Classic_All.jpg", plot=all, width = 20, height = 40, units = "cm")
...
all
ggsave("Singhal_BF_Classic_Tox.jpg", plot=all, width = 20, height = 30, units = "cm")
