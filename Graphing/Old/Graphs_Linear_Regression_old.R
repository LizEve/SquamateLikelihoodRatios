...
burbrink <- all
rm(all,all.e,all.exp,B,G,mLBF,mLGL,a,dataset)
...
quartz()
# x y
a.lr <- LR(all.d,h,cc1,70,70,20,150,150,50,0,160,"All")
a.lr

a.lr.80m <- LR(all.80m,h,cc1,60,60,10,60,50,10,0,50,"80m") 
a.lr.80m

a.lr.50m <- LR(all.50m,h,cc1,60,60,10,60,50,10,0,50,"50m") 
a.lr.50m

# named 2x in paper cuz its 2x threshold vut its +1 threshold above threshold.
a.lr.2x <- LR(all.1x,h,cc1,60,60,10,100,100,10,0,60,"2x") 
a.lr.2x

a.lr.1x <- LR(all.0x,h,cc1,60,60,10,100,100,10,0,60,"1x") 
a.lr.1x
...
m2 <- ggarrange(a.lr,a.lr.80m, ncol=2, nrow=1,align="h")


m2

figm <- annotate_figure(m2,
                        bottom = text_grob("dGLS", size = 16),
                        left = text_grob("ln(BF)",  size = 16, rot = 90),
                        top = text_grob(tt,  size = 18))
...
ggsave(paste(dataset,h,"scatter_all_scattertvs.pdf",sep="_"), plot=a.lr,width = 8, height = 8, units = "in", device = 'pdf',bg = "transparent")
ggsave(paste(dataset,h,"scatter_all80_scattertvs.pdf",sep="_"), plot=a.lr.80m,width = 8, height = 8, units = "in", device = 'pdf',bg = "transparent")
...
quartz()
a.lr <- LR(all.d,h,cc1,60,60,20,60,60,20,0,60,"")
a.lr 
 
a.lr.80m <- LR(all.80m,h,cc1,50,50,5,60,60,10,0,40,"") 
a.lr.80m

a.lr.50m <- LR(all.50m,h,cc1,40,40,2,60,60,5,0,15,"50m") 
a.lr.50m

# named 2x in paper cuz its 2x threshold vut its +1 threshold above threshold.
a.lr.2x <- LR(all.1x,h,cc1,40,40,2,60,60,5,0,15,"2x") 
a.lr.2x
...
ggsave(paste("All_scatter_lrm.pdf",sep="_"), plot=figm,width = 8, height = 8, units = "in", device = 'pdf',bg = "transparent")
ggsave(paste("All_scatter_toxall_scatterPPT.svg",sep="_"), plot=a.lr,width = 4, height = 4, units = "in", device = 'svg',bg = "transparent")
ggsave(paste("All_scatter_tox80_scatterPPT.svg",sep="_"), plot=a.lr.80m,width = 4, height = 4, units = "in", device = 'svg',bg = "transparent")
