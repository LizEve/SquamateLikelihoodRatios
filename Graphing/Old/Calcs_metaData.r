...
mLGL <- read.table(paste("/Users/ChatNoir/Projects/Squam/scripts/Graphing/DataFiles/",dataset,"MaxLikeSummaryNA.txt",sep=""),
                     header=TRUE) %>% drop_na()
hot <- read.table(paste("/Users/ChatNoir/Projects/Squam/scripts/Graphing/DataFiles/",dataset,"_HoTNA.out",sep=""),
                   header=TRUE) %>% drop_na()
mdat <- read.table(paste("/Users/ChatNoir/Projects/Squam/scripts/Graphing/DataFiles/",dataset,"_iqdataNA.txt",sep=""),
                   header=TRUE) %>% drop_na()


## List loci that didn't run through HoT 
setdiff(mLGL$Locus,hot$Locus)
...
# left join the metadata by locus name 
mL <- left_join(left_join(mL,hot,by="Locus"),mdat,by="Locus")

# check NA 
sum(is.na(mL$MEAN_COL_SCORE))
sum(is.na(mL))
