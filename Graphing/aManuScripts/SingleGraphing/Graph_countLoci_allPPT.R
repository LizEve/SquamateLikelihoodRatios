...
# set up to load both datasets to graph if you want. i haven't done that yet. 
...
TX.pos <- pos %>% filter(variable!="TvS") %>% 
  droplevels() %>%
  mutate(Hpos=factor(Hpos, levels=c("AI","SA","SI","Zero"))) %>% 
  complete(Hpos,Data.Set,variable,metric) %>% 
  replace_na(list(Percent.Loci = 0)) %>% 
...
all.pos <- bind_rows(G.TS.pos,B.TS.pos,G.TX.pos,B.TX.pos)
all.sig <- bind_rows(G.TS.sig,B.TS.sig,G.TX.sig,B.TX.sig)

fname <- paste("/Users/ChatNoir/Projects/Squam/Graphs/Tables/SupportAll.xlsx",sep='')

#write.xlsx2(as.data.frame(all.pos), file=fname, row.names=FALSE, sheetName = "Pos")
#write.xlsx2(as.data.frame(all.sig), file=fname, row.names=FALSE, sheetName = "Sig", append=TRUE)
...
BarPlot <- function(df,H,yval,f,cc,tt){
  gg <- ggplot(df, aes(x=H,y=yval,fill=f)) + 
    ...
    labs(x="",y='',fill="Topology \n supported") +
    guides(colour = guide_legend(override.aes = list(size=2))) +
    coord_cartesian(ylim=c(0,100)) + scale_y_continuous(breaks = seq(0,100,20)) +
    ggtitle(tt)# + geom_text(aes(x=H,y=yval, label = barlab), position = position_dodge(),angle = 45,vjust=-2)
...
cc <- c("#450156", "#30668d","#578529","gray37", "#b3b3b3")
bp <- BarPlot(B.TX.pos,B.TX.pos$Data.Set,B.TX.pos$Percent.Loci,B.TX.pos$Hpos,cc,"a. ln(BF) - Positive support") + 
  ...
  #geom_text(aes(x=B.TX.pos$Data.Set,y=B.TX.pos$Percent.Loci, label = B.TX.pos$Total.Loci), position = position_dodge(width=1),angle = 90,hjust=-0.2,vjust=0.55,size=2)
bp

gp <- BarPlot(G.TX.pos,G.TX.pos$Data.Set,G.TX.pos$Percent.Loci,G.TX.pos$Hpos,cc,"b. dGLS - Positive support") + 
  facet_wrap(~ G.TX.pos$variable,ncol=4,strip.position = c("bottom"))+ 
  theme(strip.text.x = element_text(margin = margin(0.1,0,0.1,0, "cm"))) #+ 
  #geom_text(aes(x=G.TX.pos$Data.Set,y=G.TX.pos$Percent.Loci, label = G.TX.pos$Total.Loci), position = position_dodge(width=1),angle = 90,hjust=-0.2,vjust=0.55,size=2)
#gp
#ggsave("_All_bar_support_TXnum.svg", plot=gp,width = 7, height = 4, units = "in", device = 'svg',bg = "transparent")
#ggsave("_All_bar_support_bf_TXnum.svg", plot=bp,width = 7, height = 4, units = "in", device = 'svg',bg = "transparent")

bs <- BarPlot(B.TX.sig,B.TX.sig$Data.Set,B.TX.sig$Percent.Loci,B.TX.sig$Hsig,cc,"c. ln(BF) - Positive support") + 
  facet_wrap(~ B.TX.sig$variable,ncol=4,strip.position = c("bottom")) + 
  theme(strip.text.x = element_text(margin = margin(0.1,0,0.1,0, "cm"))) #+
  #geom_text(aes(x=B.TX.sig$Data.Set,y=B.TX.sig$Percent.Loci, label = B.TX.sig$Total.Loci), position = position_dodge(width=1),angle = 90,hjust=-0.2,vjust=0.55,size=2)

gs <- BarPlot(G.TX.sig,G.TX.sig$Data.Set,G.TX.sig$Percent.Loci,G.TX.sig$Hsig,cc,"d. dGLS - Positive support") + 
  facet_wrap(~ G.TX.sig$variable,ncol=4,strip.position = c("bottom"))+ 
  theme(strip.text.x = element_text(margin = margin(0.1,0,0.1,0, "cm"))) #+ 
  #geom_text(aes(x=G.TX.sig$Data.Set,y=G.TX.sig$Percent.Loci, label = G.TX.sig$Total.Loci), position = position_dodge(width=1),angle = 90,hjust=-0.2,vjust=0.55,size=2)


p <- ggarrange(bp,gp,bs,gs,ncol=2, nrow=2,align="v", common.legend = TRUE, legend = "bottom")
...
#ggsave("Fig6_All_bar_support_TX.pdf", plot=fig,width = 7, height = 7, units = "in", device = 'pdf',bg = "transparent")
...
cc <- c(viridis(5)[4:5], "#b3b3b3", "gray90")
bp <- BarPlot(B.TS.pos,B.TS.pos$Data.Set,B.TS.pos$Percent.Loci,B.TS.pos$Hpos,cc,"a. ln(BF) - Positive support") + 
  facet_wrap(~ B.TS.pos$variable,ncol=4,strip.position = c("top")) + 
  theme(strip.text.x = element_blank()) #+
  #geom_text(aes(x=B.TS.pos$Data.Set,y=B.TS.pos$Percent.Loci, label = B.TS.pos$Total.Loci), position = position_dodge(width=1),angle = 90,hjust=-0.2,vjust=0.55,size=3)

gp <- BarPlot(G.TS.pos,G.TS.pos$Data.Set,G.TS.pos$Percent.Loci,G.TS.pos$Hpos,cc,"b. dGLS - Positive support") + 
  facet_wrap(~ G.TS.pos$variable,ncol=4,strip.position = c("top")) + 
  theme(strip.text.x = element_blank()) #+
  #geom_text(aes(x=G.TS.pos$Data.Set,y=G.TS.pos$Percent.Loci, label = G.TS.pos$Total.Loci), position = position_dodge(width=1),angle = 90,hjust=-0.2,vjust=0.55,size=3)

bs <- BarPlot(B.TS.sig,B.TS.sig$Data.Set,B.TS.sig$Percent.Loci,B.TS.sig$Hsig,cc,"c. ln(BF) - Strong support") + 
  facet_wrap(~ B.TS.sig$variable,ncol=4,strip.position = c("top")) + 
  theme(strip.text.x = element_blank()) #+
  #geom_text(aes(x=B.TS.sig$Data.Set,y=B.TS.sig$Percent.Loci, label = B.TS.sig$Total.Loci), position = position_dodge(width=1),angle = 90,hjust=-0.2,vjust=0.55,size=3)

gs <- BarPlot(G.TS.sig,G.TS.sig$Data.Set,G.TS.sig$Percent.Loci,G.TS.sig$Hsig,cc,"d. dGLS - Strong support") + 
  facet_wrap(~ G.TS.sig$variable,ncol=4,strip.position = c("top")) + 
  theme(strip.text.x = element_blank()) #+
  #geom_text(aes(x=G.TS.sig$Data.Set,y=G.TS.sig$Percent.Loci, label = G.TS.sig$Total.Loci), position = position_dodge(width=1),angle = 90,hjust=-0.2,vjust=0.55,size=3)
...
ggsave("SIFigx_All_bar_support_TS.pdf", plot=fig,width = 7, height = 7, units = "in", device = 'pdf',bg = "transparent")
