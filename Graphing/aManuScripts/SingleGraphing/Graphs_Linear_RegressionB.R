...
quartz()
# x y
a.lr <- LR(all.d,h,cc1,60,60,10,150,150,50,0,160,"a. TvS - All")
a.lr

a.lr.80m <- LR(all.80m,h,cc1,60,60,10,150,150,50,0,160,"b. TvS - 80m") 
a.lr.80m
...
a.lr.2x <- LR(all.1x,h,cc1,60,60,5,100,100,10,0,40,"2x") 
...
#quartz()
a.lr <- LR(all.d,h,cc1,40,40,5,60,60,10,0,35,"c. AIvSA - All")
#a.lr
 
a.lr.80m <- LR(all.80m,h,cc1,40,40,5,60,60,10,0,35,"d. AIvSA - 80m") 
#a.lr.80m

a.lr.50m <- LR(all.50m,h,cc1,40,40,5,60,60,10,0,35,"50m") 
#a.lr.50m

# named 2x in paper cuz its 2x threshold vut its +1 threshold above threshold.
a.lr.2x <- LR(all.1x,h,cc1,40,40,5,60,60,10,0,35,"2x") 
#a.lr.2x
...
figm <- annotate_figure(m2t2,
                        bottom = text_grob("dGLS", size = 16),
                        left = text_grob("ln(BF)",  size = 16, rot = 90),
                        top = text_grob("Burbrink",  size = 18))
figm
