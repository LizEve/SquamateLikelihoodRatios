...
save(results,file="corsubsampleStreicher")

all <- all %>% group_by(variable)
sn = "1k"
print(sn)
for (n in 1:reps) {
  sb <- sample_n(all, 1000)
  tvs <- corTest(sb,"TvS",sn) 
  aivsa <- corTest(sb,"AIvSA",sn)
  aivsi <- corTest(sb,"AIvSI",sn)
  savsi <- corTest(sb,"SAvSI",sn)
...
fname <- paste("/Users/ChatNoir/Projects/Squam/Graphs/Tables/AllSchmooshsubsampleBurbrink.xlsx",sep='')
