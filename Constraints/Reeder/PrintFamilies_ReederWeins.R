...
setwd("/Users/ChatNoir/Projects/Squam/scripts/Constraints/Reeder")
...
### List families ##############################################
fams <- unique(reeder$ReederTaxa.Family) 
# function to grab all taxa with family x
byfams <- function(x){
  reeder %>% filter(ReederTaxa.Family==x) %>% 
    ungroup %>%
    as.data.frame()
}
...
for (n in f){
  ...
  family <- unique(a$ReederTaxa.Family)
  # get taxa 
  taxa <- a$ReederTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
...
### List Clades #############################################
fams <- 0
fams <- unique(reeder$ReederTaxa.Clade) 
# function to grab all taxa with family x
byfams <- function(x){
  reeder %>% filter(ReederTaxa.Clade==x) %>% 
    ungroup %>%
    as.data.frame()
}
# List of dataframes, parsed by family 
...
for (n in f){
  ...
  a <- l[[n]]
  # extract the family name
  family <- unique(a$ReederTaxa.Clade)
  # get taxa 
  taxa <- a$ReederTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
    # Add family name to dataframe
...
### List SnakeClade #############################################
fams <- 0
fams <- unique(reeder$ReederTaxa.SnakeClade) 
# function to grab all taxa with family x
byfams <- function(x){
  reeder %>% filter(ReederTaxa.SnakeClade==x) %>% 
    ungroup %>%
    as.data.frame()
}
# List of dataframes, parsed by family 
...
for (n in f){
  ...
  a <- l[[n]]
  # extract the family name
  family <- unique(a$ReederTaxa.SnakeClade)
  # get taxa 
  taxa <- a$ReederTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
    # Add family name to dataframe
...
### List Scleroglossa #############################################
fams <- 0
fams <- unique(reeder$ReederTaxa.Scleroglossa) 
# function to grab all taxa with family x
byfams <- function(x){
  reeder %>% filter(ReederTaxa.Scleroglossa==x) %>% 
    ungroup %>%
    as.data.frame()
}
# List of dataframes, parsed by family 
...
for (n in f){
  ...
  a <- l[[n]]
  # extract the family name
  family <- unique(a$ReederTaxa.Scleroglossa)
  # get taxa 
  taxa <- a$ReederTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
    # Add family name to dataframe
...
### List ToxPoly #############################################
fams <- 0
fams <- unique(reeder$ReederTaxa.ToxPoly) 
# function to grab all taxa with family x
byfams <- function(x){
  reeder %>% filter(ReederTaxa.ToxPoly==x) %>% 
    ungroup %>%
    as.data.frame()
}
# List of dataframes, parsed by family 
...
for (n in f){
  ...
  a <- l[[n]]
  # extract the family name
  family <- unique(a$ReederTaxa.ToxPoly)
  # get taxa 
  taxa <- a$ReederTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
    # Add family name to dataframe
...
### List ToxAI #############################################
fams <- 0
fams <- unique(reeder$ReederTaxa.ToxAI) 
# function to grab all taxa with family x
byfams <- function(x){
  reeder %>% filter(ReederTaxa.ToxAI==x) %>% 
    ungroup %>%
    as.data.frame()
}
# List of dataframes, parsed by family 
...
for (n in f){
  ...
  a <- l[[n]]
  # extract the family name
  family <- unique(a$ReederTaxa.ToxAI)
  # get taxa 
  taxa <- a$ReederTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
    # Add family name to dataframe
...
### List ToxSA #############################################
fams <- 0
fams <- unique(reeder$ReederTaxa.ToxSA) 
# function to grab all taxa with family x
byfams <- function(x){
  reeder %>% filter(ReederTaxa.ToxSA==x) %>% 
    ungroup %>%
    as.data.frame()
}
# List of dataframes, parsed by family 
...
for (n in f){
  ...
  a <- l[[n]]
  # extract the family name
  family <- unique(a$ReederTaxa.ToxSA)
  # get taxa 
  taxa <- a$ReederTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
    # Add family name to dataframe
...
### List ToxSI #############################################
fams <- 0
fams <- unique(reeder$ReederTaxa.ToxSI) 
# function to grab all taxa with family x
byfams <- function(x){
  reeder %>% filter(ReederTaxa.ToxSI==x) %>% 
    ungroup %>%
    as.data.frame()
}
# List of dataframes, parsed by family 
...
for (n in f){
  ...
  a <- l[[n]]
  # extract the family name
  family <- unique(a$ReederTaxa.ToxSI)
  # get taxa 
  taxa <- a$ReederTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
    # Add family name to dataframe
...
makeFile = function(df){
  fname = paste('Reeder_',deparse(substitute(df)),'_iq.txt',sep='')
...
x <- z[!duplicated(z)]
write(x,file="Reeder_ConstraintNames.txt",append=FALSE)
