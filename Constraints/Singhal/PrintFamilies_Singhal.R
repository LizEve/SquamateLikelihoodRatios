...
singhal <- as.data.frame(x[6])
reeder <- as.data.frame(x[5])
...
### List families ##############################################
fams <- unique(singhal$SinghalTaxa.Family) 
# function to grab all taxa with family x
byfams <- function(x){
  singhal %>% filter(SinghalTaxa.Family==x) %>% 
    ungroup %>%
    as.data.frame()
}
...
for (n in f){
  ...
  family <- unique(a$SinghalTaxa.Family)
  # get taxa 
  taxa <- a$SinghalTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
...
### List Clades #############################################
fams <- 0
fams <- unique(singhal$SinghalTaxa.Clade) 
# function to grab all taxa with family x
byfams <- function(x){
  singhal %>% filter(SinghalTaxa.Clade==x) %>% 
    ungroup %>%
    as.data.frame()
}
# List of dataframes, parsed by family 
...
for (n in f){
  ...
  a <- l[[n]]
  # extract the family name
  family <- unique(a$SinghalTaxa.Clade)
  # get taxa 
  taxa <- a$SinghalTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
    # Add family name to dataframe
...
### List SnakeClade #############################################
fams <- 0
fams <- unique(singhal$SinghalTaxa.SnakeClade) 
# function to grab all taxa with family x
byfams <- function(x){
  singhal %>% filter(SinghalTaxa.SnakeClade==x) %>% 
    ungroup %>%
    as.data.frame()
}
# List of dataframes, parsed by family 
...
for (n in f){
  ...
  a <- l[[n]]
  # extract the family name
  family <- unique(a$SinghalTaxa.SnakeClade)
  # get taxa 
  taxa <- a$SinghalTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
    # Add family name to dataframe
...
### List Scleroglossa #############################################
fams <- 0
fams <- unique(singhal$SinghalTaxa.Scleroglossa) 
# function to grab all taxa with family x
byfams <- function(x){
  singhal %>% filter(SinghalTaxa.Scleroglossa==x) %>% 
    ungroup %>%
    as.data.frame()
}
# List of dataframes, parsed by family 
...
for (n in f){
  ...
  a <- l[[n]]
  # extract the family name
  family <- unique(a$SinghalTaxa.Scleroglossa)
  # get taxa 
  taxa <- a$SinghalTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
    # Add family name to dataframe
...
### List ToxPoly #############################################
fams <- 0
fams <- unique(singhal$SinghalTaxa.ToxPoly) 
# function to grab all taxa with family x
byfams <- function(x){
  singhal %>% filter(SinghalTaxa.ToxPoly==x) %>% 
    ungroup %>%
    as.data.frame()
}
# List of dataframes, parsed by family 
...
for (n in f){
  ...
  a <- l[[n]]
  # extract the family name
  family <- unique(a$SinghalTaxa.ToxPoly)
  # get taxa 
  taxa <- a$SinghalTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
    # Add family name to dataframe
...
### List ToxAI #############################################
fams <- 0
fams <- unique(singhal$SinghalTaxa.ToxAI) 
# function to grab all taxa with family x
byfams <- function(x){
  singhal %>% filter(SinghalTaxa.ToxAI==x) %>% 
    ungroup %>%
    as.data.frame()
}
# List of dataframes, parsed by family 
...
for (n in f){
  ...
  a <- l[[n]]
  # extract the family name
  family <- unique(a$SinghalTaxa.ToxAI)
  # get taxa 
  taxa <- a$SinghalTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
    # Add family name to dataframe
...
### List ToxSA #############################################
fams <- 0
fams <- unique(singhal$SinghalTaxa.ToxSA) 
# function to grab all taxa with family x
byfams <- function(x){
  singhal %>% filter(SinghalTaxa.ToxSA==x) %>% 
    ungroup %>%
    as.data.frame()
}
# List of dataframes, parsed by family 
...
for (n in f){
  ...
  a <- l[[n]]
  # extract the family name
  family <- unique(a$SinghalTaxa.ToxSA)
  # get taxa 
  taxa <- a$SinghalTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
    # Add family name to dataframe
...
### List ToxSI #############################################
fams <- 0
fams <- unique(singhal$SinghalTaxa.ToxSI) 
# function to grab all taxa with family x
byfams <- function(x){
  singhal %>% filter(SinghalTaxa.ToxSI==x) %>% 
    ungroup %>%
    as.data.frame()
}
# List of dataframes, parsed by family 
...
for (n in f){
  ...
  a <- l[[n]]
  # extract the family name
  family <- unique(a$SinghalTaxa.ToxSI)
  # get taxa 
  taxa <- a$SinghalTaxa.Original_names_of_sequences
  if (length(taxa) > 1) {
    # Add family name to dataframe
...
newer <- newer[c("ConstraintName","TaxaListClean")]
...
makeFile = function(df){
  fname = paste('Singhal_',deparse(substitute(df)),'_iq.txt',sep='')
...
x <- z[!duplicated(z)]
write(x,file="Singhal_ConstraintNames.txt",append=FALSE)
