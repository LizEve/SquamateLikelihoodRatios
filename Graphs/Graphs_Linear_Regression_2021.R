...
df.m <- full_join(full_join(all.60m,all.80m,by=c('Locus', "variable", "dataSet"), suffix = c("60m","80m")), all, by=c('Locus', "variable", "dataSet"))

df.mb <- full_join(full_join(all.60mb,all.80mb,by=c('Locus', "variable", "dataSet"), suffix = c("60mb","80mb")), all, by=c('Locus', "variable", "dataSet"))
...
scat1 <- ggplot(df1) + 
  geom_point(aes(GL,BF),alpha=0.75,size=0.5,color="#B63679FF") + 
  geom_point(aes(GL80m, BF80m),alpha=0.95,size=0.5,color="#826fff") + 
  geom_point(aes(GL60m, BF60m),alpha=0.95,size=0.5,color="#4B4AA2") + 
...
s <- ggarrange(scat1,scat2,scat3,scat4, ncol=4, nrow=1,align="hv", legend = none, legend = "right")
...
scat <- ggplot(df) + 
  geom_point(aes(GL,BF),alpha=0.75,size=0.5,color="#B63679FF") + 
  geom_point(aes(GL80m, BF80m),alpha=0.85,size=0.5,color="#826fff") + 
  geom_point(aes(GL60m, BF60m),alpha=0.95,size=0.5,color="#4B4AA2") + 
  geom_smooth(aes(GL,BF), method=lm, formula = y ~ x,size=0.25,se=F, color="#830346",fullrange = T) +
  geom_smooth(aes(GL80m,BF80m), method=lm,formula = y ~ x,size=0.25,se=F,color="#6956E6",fullrange = T) +
  geom_smooth(aes(GL60m,BF60m), method=lm,formula = y ~ x,size=0.25,se=F, color="#323189",fullrange = T) +
...
ggsave("Scatter_tvs_bydataset1.4.pdf", plot=fig,width = 7, height = 1.4, units = "in", device = 'pdf',bg = "transparent")
ggsave("Scatter_tvs_bydataset1.6.pdf", plot=fig,width = 7, height = 1.6, units = "in", device = 'pdf',bg = "transparent")
...
ggsave("Scatter_tvs_bydataset.pdf", plot=fig,width = 7, height = 1.4, units = "in", device = 'pdf',bg = "transparent")
