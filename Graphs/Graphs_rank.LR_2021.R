...
df1 <- all %>% filter(dataSet == "Burbrink") 
df2 <- all %>% filter(dataSet == "Reeder") 
df3 <- all %>% filter(dataSet == "Singhal") 
df4 <- all %>% filter(dataSet == "Streicher") 
...
st1 <- ggscatter(df1, y = "drank.b", x = "drank.g",
                 color = "drank.diff.n", alpha=0.75,size=0.75, 
                 add = "reg.line", fullrange = FALSE,
                 add.params = list(color = "blue", size=0.25)) + 
  mytheme +
  facet_grid(variable~dataSet,scales = "free") + 
  stat_cor(aes(color = drank.diff.n,label =  ..rr.label.., size=0.1),
           label.y = 4, label.x = 150, size=2, r.accuracy = 0.001)+
  theme(strip.background = element_blank(),strip.text.y = element_blank()) 
st1 + scale_color_viridis(limits=c(0,100),direction = -1,option='D')


st2 <- ggscatter(df2, y = "drank.b", x = "drank.g",
                 color = "drank.diff.n", alpha=0.75,size=0.5, 
                 add = "reg.line", fullrange = FALSE) + mytheme +
  facet_grid(variable~dataSet,scales = "free") + 
  stat_cor(aes(color = drank.diff.n,label =  ..rr.label..),
           label.y = 4, label.x = 30, size=2, r.accuracy = 0.001)+
  theme(strip.background = element_blank(),strip.text.y = element_blank()) 
st2+scale_color_viridis(limits=c(0,100),direction = -1,option='D')

st3 <- ggscatter(df3, y = "drank.b", x = "drank.g",
                 color = "drank.diff.n", alpha=0.75,size=0.5, 
                 add = "reg.line", fullrange = FALSE,
                 add.params = list(color = "blue", size=0.25)) +
  mytheme +
  ...
  stat_cor(aes(color = drank.diff.n,label =  ..rr.label..),
           label.y = 4, label.x = 150, size=2, r.accuracy = 0.001)+
  theme(strip.background = element_blank(),strip.text.y = element_blank()) 
st3+scale_color_viridis(limits=c(0,100),direction = -1,option='D')


st4 <- ggscatter(df4, y = "drank.b", x = "drank.g",
                 color = "drank.diff.n", alpha=0.5,size=0.5, 
                 add = "reg.line", fullrange = FALSE) + mytheme +
  facet_grid(variable~dataSet,scales = "free") + 
  stat_cor(aes(color = drank.diff.n,label =  ..rr.label..),
           label.y = 4, label.x = 150, size=2, r.accuracy = 0.001)+
  theme(strip.background = element_blank(),strip.text.y = element_blank()) 
st4+scale_color_viridis(limits=c(0,100),direction = -1,option='D')
